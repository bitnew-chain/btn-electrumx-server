...
class ElectrumX(SessionBase):
    ...
    async def hash160_contract_status(self, hash160, contract_addr, topic):
        eventlogs = await self.controller.contract_event_get_history(hash160, contract_addr, topic)
    ...
    def block_headers(self, start_height, count):
        ...
        At most MAX_CHUNK_SIZE headers will be returned.'''
    ...
    async def replaced_banner(self, banner):
        ...
        server_version = self.controller.VERSION.split()[-1]
        for pair in [
                ('$VERSION', self.controller.VERSION), # legacy
    ...
    async def banner(self):
        ...
        banner = 'Welcome to BTN Electrum!'
    ...
    def server_version(self, client_name=None, protocol_version=None):
        ...
        # PROTOCOL_MAX
        if ptuple is None or (ptuple > (1, 5) and protocol_version is None):
    ...
    def set_protocol_handlers(self, ptuple):
        ...
        if ptuple >= (1, 3):
            handlers.update({
                ...
                'blockchain.hash160.contract.get_eventlogs': controller.contract_event_get_history_1_3,
                'blockchain.hash160.contract.subscribe': self.contract_event_subscribe_1_3,
            })

        if ptuple >= (1, 4):
            handlers.update({
                'blockchain.contract.event.get_history': controller.contract_event_get_history,
