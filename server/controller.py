...
class Controller(ServerBase):
    ...
    def __init__(self, env):
        ...
        cmds = ('daemon_url disconnect getinfo groups log reorg '
                'sessions stop'.split())
    ...
    def notify_sessions(self, touched, eventlog_touched=None):
        ...
        for session in self.sessions:
            ...
            if session_touched is not None or session_eventlog_touched is not None:
                self.create_task(session.notify_async(session_touched, session_eventlog_touched))
    ...
    def electrum_header(self, height):
        ...
        if height not in self.header_cache:
            ...
            self.header_cache[height] = self.coin.electrum_header(raw_header, height)
    ...
    async def contract_event_get_history(self, hash160, contract_addr, topic):
        ...
        conf = [{'tx_hash': hash_to_str(tx_hash),
                 'height': height,
                 'log_index': log_index}
