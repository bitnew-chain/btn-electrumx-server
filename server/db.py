...
class DB(object):
    ...
    def open_dbs(self):
        ...
        self.logger.info('eventlog flush count: {:,d}'.format(self.eventlog_flush_count))
        if self.first_sync:
    ...
    def _compact_hashX(self, hashX, hist_map, hist_list,
        ...
        if nrows > 4:
            self.logger.infp('hashX {} is large: {:,d} entries across {:,d} rows'
    ...
    def flush_eventlogs(self, eventlogs):
        self.eventlog_flush_count += 1
        flush_id = pack('>H', self.eventlog_flush_count)

        with self.eventlog_db.write_batch() as batch:
            for hashY in sorted(eventlogs):
                key = hashY + flush_id
                # 把二维数据按照一维数组存储
                batch.put(key, b''.join([x.tobytes() for x in eventlogs[hashY]]))
            self.write_eventlog_state(batch)

    def backup_eventlogs(self, hashY_keys):
        # Not certain this is needed, but it doesn't hurt
        self.eventlog_flush_count += 1
        nremoves = 0
        ...
        with self.eventlog_db.write_batch() as batch:
            for addr in sorted(hashY_keys):
                ...
                for key, hist in self.eventlog_db.iterator(prefix=addr, reverse=True):
                    ...
                    idx = bisect_left(tx_nums, self.tx_count)
    ...
    def get_eventlog(self, key, limit=1000):
        ...
        :return:
        """
        for tx_num, log_index in self.get_eventlog_txnums(key, limit):
