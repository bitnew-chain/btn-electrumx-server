...
from lib.hash import hash_to_str, hash_to_hex_str, HASHX_LEN
...
class BlockProcessor(server.db.DB):
    ...
    def __init__(self, env, controller, daemon):
        ...
        # eventlogs
        self.eventlogs = defaultdict(list)  # {b'hashY' => [array('I', [txnum, log_index]),]}
    ...
    async def reorg_chain(self, count=None):
        ...
        for hex_hashes in chunks(hashes, 50):
            ...
            await self.controller.run_in_executor(self.backup_blocks, blocks, eventlog_hashYs)
        await self.prefetcher.reset_height()
    ...
    def flush(self, flush_utxos=False):
        ...
        # eventlog
        self.flush_eventlogs(self.eventlogs)
        if self.utxo_db.for_sync:
    ...
    def advance_blocks(self, blocks, eventlogs):
        ...
        height = self.height
        eventlog_dict, hashY_dict = self.eventlogs_to_dict(eventlogs)
        self.hashYs = hashY_dict
    ...
    def backup_blocks(self, raw_blocks, eventlog_hashYs):
